...
Delivery: `chrome.runtime.sendMessage(EXTENSION_ID, { action: "scrapeConversation" }, callback)`. Already works today.
...
Four subsections, rendered as read-only cards:
...
Debug-only visibility: always shown (useful for the admin reviewing why
a response went sideways).
...
### Extension contract (no changes)

The extension already exposes:

- `chrome.runtime.sendMessage(EXTENSION_ID, { action: "scrapeConversation" }, cb)` → `{success, messages}`
- `chrome.storage.local.rm_matched_resort` → `{id, name, ...}` (populated by content script when it detects a matching resort)

For the matched resort, `ai.html` will `chrome.storage.local.get("rm_matched_resort")` directly if the extension is external, or via `sendMessage({ action: "getMatchedResort" })` if we add one handler.

**Decision:** add one new extension handler `getMatchedResort` that returns the stored value. Cleaner API than having `ai.html` poke extension storage directly, and avoids permission issues.
...
- `extension/content.js` — add handler for `getMatchedResort` action that returns the value from `chrome.storage.local.rm_matched_resort`.
- `extension/background.js` — if the handler lives in background (MV3 service worker), add it there and relay to the correct context.
...
- `template.html`, `login.html` — unrelated.
...
1. **Commit A — Visual refresh port.** Cherry-pick / reapply the Ink Navy palette + Inter font + refined components (`style/style.css` + per-screen CSS files) from `claude/create-test-branch-CRHfr`. The HTML files on `main` are Supabase-based so the CSS applies cleanly — no JS changes needed.
